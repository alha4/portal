	...
	elseif ($_POST['PULL_UPDATE_STATE'] == 'Y')
	{
		$arMessage = CPullStack::Get($_POST['CHANNEL_ID'], intval($_POST['CHANNEL_LAST_ID']));
                ...
                if(is_int($NEWID)) {
                  ...
                  $addedUser     = CUser::GetByID($NEWID);
                  $addedUserRs   = $addedUser->Fetch();
                  $departamentID = array_pop($addedUserRs['UF_DEPARTMENT']);

                  $currUser = CUser::GetByID($USER->GetID());
                  $currUserRs = $currUser->Fetch();
                  $currDepartamentID = $currUserRs['UF_DEPARTMENT'][0];

                  if($departamentID != $currDepartamentID) {
